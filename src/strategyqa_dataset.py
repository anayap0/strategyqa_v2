...
class StrategyQADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        encoded_inputs = self.tokenizer(question, decomposition, return_tensors='pt', padding=True, truncation=True, max_length=self.max_length)
        ...
        for evidences in evidence:
            ...
            for fact in evidences:
                if isinstance(fact, list):  # If fact is a list of strings
                    encoded_fact = self.tokenizer.encode_plus(fact, return_tensors='pt', padding=True, truncation=True, max_length=self.max_length)
                    encoded_facts.append(encoded_fact)
                else:  # If fact is a string indicating operation
                    encoded_facts.append(fact)
